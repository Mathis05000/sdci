package com.example.executor;

import java.io.BufferedReader;
import java.io.InputStreamReader;
import java.io.OutputStream;
import java.net.HttpURLConnection;
import java.net.URI;
import java.net.URL;
import java.net.http.HttpClient;
import java.net.http.HttpRequest;
import java.net.http.HttpRequest.BodyPublishers;
import java.net.http.HttpResponse;
import java.nio.charset.StandardCharsets;
import java.util.concurrent.BlockingQueue;

import org.json.JSONObject;

public class Executor {

    private BlockingQueue<String> dataQueueIn;  // File pour recevoir les commandes SCALE_UP / SCALE_DOWN
    private static final String API_URL = "https://192.168.10.101:6443/apis/apps/v1/namespaces/default/deployments/sdci-gwi/scale";
    private static final String TOKEN = "eyJhbGciOiJSUzI1NiIsImtpZCI6InRzX0hFWkRtWWRxWE5SY1M4bzlFeXVMTWdBaXJ5Tl83Vi03MWRrZ1Z3VmMifQ..esGLfjJzbBbOjpWrE1m-psYOK-Zan7X_LQZ04yr_L6qUe10igmXq9nuiNkzgom7N6Q5tVp_cU6sigKJdi-GOilhjBdqkxQmM2LIfn-4ZEQs2nyKaMNykLmYCr2tGyYclamBMUJGeeo3G3HFFekeGdM_uCl2L09LhUeDognJZupkUuxYFEnELbMkVDH2mYjFNHNRCG8a3HaSeNL7_UhWEMTnw6hAjORJGBXFge-NvidCznyt-3cBQ6jSMqX_quJZ1AgyhX-EUIxiZN9bjQCnM9ejrZc-TcDFVMI5RFXQicaPSYhphVPQIfj3YpWO_RDBy2zs7yvDs6rB0kZIT8Nnezg";  // Remplacez par votre token d'authentification

    public Executor(BlockingQueue<String> dataQueueIn) {
        this.dataQueueIn = dataQueueIn;
    }

    public void startExecuting() {
        // Tâche d'exécution qui attend indéfiniment des commandes de mise à l'échelle
        Runnable task = () -> {
            while (true) {
                try {
                    // Attendre une commande de la file
                    String command = dataQueueIn.take();
                    // Exécuter la commande en fonction de la valeur
                    executeCommand(command);
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
            }
        };

        // Exécuter le thread d'exécution
        new Thread(task).start();
    }

    private void executeCommand(String command) {
        try {
            // Faire une requête GET pour obtenir le nombre actuel de réplicas
            int currentReplicas = getCurrentReplicas();

            // Calculer le nouveau nombre de réplicas
            int newReplicas = (command.equals("SCALE_UP_PLAN")) ? currentReplicas + 1 : currentReplicas - 1;

            // Envoyer la requête PATCH pour mettre à jour le nombre de réplicas
            updateReplicas(newReplicas);

        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    private int getCurrentReplicas() throws Exception {
        // Faire une requête GET pour obtenir l'état actuel du Deployment
        URL url = new URL(API_URL);
        HttpURLConnection connection = (HttpURLConnection) url.openConnection();
        connection.setRequestMethod("GET");
        connection.setRequestProperty("Authorization", "Bearer " + TOKEN);

        // Lire la réponse de l'API
        BufferedReader reader = new BufferedReader(new InputStreamReader(connection.getInputStream()));
        String inputLine;
        StringBuilder response = new StringBuilder();
        while ((inputLine = reader.readLine()) != null) {
            response.append(inputLine);
        }
        reader.close();

        JSONObject responseJson = new JSONObject(response.toString());

        // Extraire le nombre de réplicas de la réponse JSON
        // En supposant que le nombre de réplicas se trouve sous "status" -> "replicas"
        int currentReplicas = responseJson.getJSONObject("status").getInt("replicas");

        return currentReplicas;
    }
    public void updateReplicas(int newReplicas) {

        if (newReplicas > 0) {
            try {
                // Créer le corps de la requête PATCH pour mettre à jour le nombre de réplicas
                String replicasJson = "[{\"op\": \"replace\",\"path\": \"/spec/replicas\",\"value\": " + newReplicas + "}]";
    
                // Créer une requête HTTP PATCH
                HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(API_URL))
                    .header("Authorization", "Bearer " + TOKEN)
                    .header("Content-Type", "application/json-patch+json")
                    .method("PATCH", BodyPublishers.ofString(replicasJson))
                    .build();
    
                // Envoyer la requête et obtenir la réponse
                HttpClient client = HttpClient.newHttpClient();
                HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString());
    
                // Vérifier la réponse
                int responseCode = response.statusCode();
                if (responseCode == 200) {
                    System.out.println("Échelle mise à jour avec succès : " + newReplicas + " réplicas.");
                    response.body();
                } else {
                    System.out.println("Erreur lors de la mise à l'échelle : " + responseCode);
                    System.out.println("Erreur détaillée : " + response.body());
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        
    }
}
